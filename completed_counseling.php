...
// Replace the SQL query construction (around line 47-63) with this code
$sql = "SELECT cs.*, c.counselors_name, 
        CASE WHEN v.full_name IS NOT NULL THEN 1 ELSE cs.with_violation END as with_violation,
        ...
        LEFT JOIN counselors c ON cs.counselors_id = c.counselors_id
        LEFT JOIN violations v ON cs.student_full_name = v.full_name 
            AND cs.year_and_section = v.year_and_section
...
<head>
    <meta charset="UTF-8">
    <title>Completed Counseling</title>
    <!-- Oservefavicon -->
...
<li>
    ...
    <li>
        ...
        <div id="content">
            <div class="menu-header">
                ...
                <span class="menu-text"> Completed Counseling</span>
                <div class="col py-3">
 ...
 <div class="d-flex justify-content-between align-items-center">
    ...
    Walk-in</button>
        <form class="form-inline d-flex align-items-center mr-2" method="GET" action="completed_counseling.php">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control text-uppercase" id="studentFullName" name="student_full_name" required 
...
<header id="printHeader" class="d-none d-print-block">
      ...
      <!-- Replace the current table with this simplified version -->
<!-- Replace the current table with this simplified version -->
            ...
            <?php foreach ($counselingSessions as $session): ?>
                <tr>
                    ...
                    <td class="status-<?php echo strtolower(htmlspecialchars($session['status'])); ?>">
                        <?= htmlspecialchars($session['status']) ?: 'N/A'; ?>
                    </td>
                    <td>
                        <?php 
                    ...
                    <td class="text-center">
                        <!-- View Details Button -->
                        <button type="button" class="btn btn-info" data-toggle="modal" data-target="#detailsModal_<?= $session['counseling_id']; ?>">
                            <i class="bi bi-eye"></i>
                        </button>
                    </td>
                    <td class="action-buttons" style="justify-content: center;">
                        <!-- Edit Button -->
                        <a href="edit_counseling.php?id=<?= $session['counseling_id']; ?>" class="edit-btn btn btn-md btn-outline-success">
                        ...
                        <!-- Archive Button -->
                        <button type="button" 
                                class="arc-btn btn btn-outline-danger archive-counseling-btn" 
...
<!-- Add this section for the detailed modals -->
<?php foreach ($counselingSessions as $session): ?>
    <!-- Details Modal -->
    <div class="modal fade" id="detailsModal_<?= $session['counseling_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="detailsModalLabel_<?= $session['counseling_id']; ?>" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header bg-info text-white">
                    <h5 class="modal-title" id="detailsModalLabel_<?= $session['counseling_id']; ?>" style="font-weight: bold;">
                        Counseling Details
                        <!-- : <?= htmlspecialchars($session['student_full_name']); ?> -->
                    </h5>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <p><strong>Full Name:</strong> <?= htmlspecialchars($session['student_full_name']); ?></p>
                            <p><strong>Year and Section:</strong> <?= htmlspecialchars($session['year_and_section']); ?></p>
                            <p><strong>With Violation:</strong> 
                                <?= $session['with_violation'] ? '<span class="text-success"> Yes</span>' : '<span class="text-dark font-weight-bold">No</span>'; ?>
                            </p>
                            <p><strong>Counselor:</strong> <?= htmlspecialchars($session['counselors_name']) ?: 'N/A'; ?></p>
                            <p><strong>Status:</strong> <?= htmlspecialchars($session['status']); ?></p>
                            <p><strong>Date Created:</strong> <?= (new DateTime($session['timestamp']))->format('M d, Y h:i A'); ?></p>
                        </div>
                        
                        <div class="col-md-6">
                            <?php if ($session['with_violation']): ?>
                                <h4 class="font-weight-bold mb-3">Violation Information</h4>
                                <?php
                                $studentViolations = explode(',', $session['violation_types']);
                                $violationInfo = explode(',', $session['violation_details']);
                                
                                if (!empty($studentViolations[0])) {
                                    for ($i = 0; $i < count($studentViolations); $i++) {
                                        echo '<p><strong>Type of Violation:</strong> ' . htmlspecialchars(trim($studentViolations[$i])) . '</p>';
                                        echo '<p><strong>Details:</strong> ' . htmlspecialchars(trim($violationInfo[$i] ?? '')) . '</p>';
                                    }
                                } else {
                    ...
                    <div class="row mt-3">
                        <div class="col-md-12">
                            <h4 class="font-weight-bold mb-3">Attached File</h4>
                            <?php if (!empty($session['file_name'])): ?>
                                <p><a href="<?= $session['file_path']; ?>" target="_blank" class="btn btn-md btn-primary">
                                    <i class="fas fa-file"></i> <?= htmlspecialchars($session['file_name']); ?>
                                </a></p>
                            <?php else: ?>
                                <p>No file attached</p>
                                
                                <!-- File Upload Form -->
                                <form action="upload_file_or_paragraph.php" method="post" enctype="multipart/form-data" class="file-upload-form">
                                    <input type="hidden" name="counseling_id" value="<?= $session['counseling_id']; ?>">
                                    <div class="custom-file-upload">
                                        <label for="file_modal_<?= $session['counseling_id']; ?>" class="choose-btn btn btn-outline-secondary file-label">Choose File</label>
                                        <span id="fileName_modal_<?= $session['counseling_id']; ?>">No file chosen</span>
                                        <input type="file" name="file" id="file_modal_<?= $session['counseling_id']; ?>" class="file-input d-none" accept="image/*,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet">
                                    </div>
                                    <button type="submit" class="btn btn-success btn-md mt-2 upload-btn" disabled>Upload</button>
                                </form>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="row mt-3">
                        <div class="col-md-12">
                            <h4 class="font-weight-bold mb-3">Remarks</h4>
                            <div class="form-group">
                                <textarea class="form-control" rows="3" disabled><?= !empty($session['remarks']) ? htmlspecialchars($session['remarks']) : 'No remarks available' ?></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row mt-3">
                        <div class="col-md-12">
                            <h4 class="font-weight-bold mb-3">Additional Details</h4>
                            <?php if (isset($_SESSION['paragraphs'][$session['counseling_id']])): ?>
                                <?php foreach ($_SESSION['paragraphs'][$session['counseling_id']] as $index => $paragraph): ?>
                                    <div class="form-group">
                                        <label><strong>Detail <?= $index + 1 ?>:</strong></label>
                                        <textarea class="form-control" rows="3" disabled><?= htmlspecialchars($paragraph) ?></textarea>
                                    </div>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <div class="form-group">
                                    <textarea class="form-control" rows="3" disabled>No additional details available</textarea>
                                </div>

                                <!-- Add Details Form -->
                                <form action="<?= $_SERVER['PHP_SELF']; ?>" method="post" class="paragraph-form mt-2">
                                    <input type="hidden" name="counseling_id" value="<?= $session['counseling_id']; ?>">
                                    <div class="form-group">
                                        <textarea name="paragraph" class="form-control" rows="3" placeholder="Add details here..." required></textarea>
                                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <a href="edit_counseling.php?id=<?= $session['counseling_id']; ?>" class="btn btn-success">Edit</a>
                </div>
            </div>
        </div>
    </div>
...
document.getElementById('customFileInput').addEventListener('change', function (event) {
    ...
    if (!allowedExtensions.includes(fileExtension)) {
        ...
        alertMessage.textContent = 'Invalid file type. Please upload a valid file.';
         // Automatically hide the alert after 5 seconds
         setTimeout(() => {
            alertMessage.classList.add('fade-out');
            setTimeout(() => {
                alertMessage.style.display = 'none'; // Hide after fade-out
                alertMessage.classList.remove('fade-out'); // Reset for next use
            }, 500); // Wait for fade-out transition to complete
        }, 5000); // 5 seconds delay
        fileInput.value = ''; // Clear the input
        fileNameDisplay.textContent = 'No file chosen';
    } else {
        uploadBtn.disabled = false; // Enable upload button if valid
    }
});
</script>
...
<div class="modal fade" id="actionConfirmationModal" tabindex="-1" role="dialog" aria-labelledby="actionConfirmationModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        ...
        <!-- Add Paragraph Modal -->
 <?php foreach ($counselingSessions as $session): ?>
        <div class="modal fade" id="paragraphModal_<?= $session['counseling_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="paragraphModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="paragraphModalLabel">Type other details</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form action="<?= $_SERVER['PHP_SELF']; ?>" method="post" class="paragraph-form">
                            <input type="hidden" name="counseling_id" value="<?= $session['counseling_id']; ?>">
                            <textarea name="paragraph" class="form-control" rows="3" placeholder="Type your paragraph here..." required></textarea>
        ...
        <!-- View Paragraphs Modal -->
        <?php if (isset($_SESSION['paragraphs'][$session['counseling_id']])): ?>
            <div class="modal fade" id="viewParagraphsModal_<?= $session['counseling_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="viewParagraphsModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="viewParagraphsModalLabel">View Details</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <?php foreach ($_SESSION['paragraphs'][$session['counseling_id']] as $index => $paragraph): ?>
                                <p style="color: #000000;"><strong>Detail <?= $index + 1 ?>:</strong> <?= htmlspecialchars($paragraph) ?></p>
                            <?php endforeach; ?>
                        </div>
                    </div>
                </div>
...
$(document).ready(function() {
    $('.file-input').on('change', function() {
        ...
        const fileNameDisplay = form.find('span[id^="fileName_"]');
...
$(document).ready(function() {
    // Handle file inputs in modals
    $('.file-input').on('change', function() {
        const fileInput = $(this);
        const form = fileInput.closest('form');
        const fileName = fileInput.val().split('\\').pop();
        const uploadBtn = form.find('.upload-btn');
        const fileNameDisplay = form.find('span[id^="fileName_modal_"]');
