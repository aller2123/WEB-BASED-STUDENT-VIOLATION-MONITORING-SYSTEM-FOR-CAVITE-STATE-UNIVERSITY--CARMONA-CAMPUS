...
$sql = "SELECT cs.*, c.counselors_name,
    CASE WHEN COUNT(v.full_name) > 0 THEN 1 ELSE cs.with_violation END as with_violation,
...
$sql .= " AND cs.counseling_id IN (
    SELECT MAX(counseling_id)
    FROM counseling_sessions
    WHERE status = 'Scheduled'
    GROUP BY student_full_name
)
GROUP BY cs.counseling_id
...
<head>
    <meta charset="UTF-8">
    <title>Scheduled Counseling</title>
    <!-- Oservefavicon -->
...
<li>
    ...
    <li>
        ...
        <div id="content">
            <div class="menu-header">
                ...
                <span class="menu-text"> Scheduled Counseling</span>
                <div class="col py-3">
 ...
 <div class="d-flex justify-content-between align-items-center">
    ...
    Walk-in</button>
        <form class="form-inline d-flex align-items-center mr-2" method="GET" action="scheduled_counseling.php">
...
<td><?= htmlspecialchars($session['phone_number'] ?? 'N/A') ?></td>
<td>
    <?php if (!empty($session['email'])): ?>
        <a href="#" class="btn btn-primary" data-toggle="modal" data-target="#emailDetailModal<?= $session['counseling_id']; ?>">
    <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" fill="currentColor" class="bi bi-eye" viewBox="0 0 16 16">
        ...
        <!-- Email Detail Modal -->
        <div class="modal fade" id="emailDetailModal<?= $session['counseling_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="emailDetailModalLabel<?= $session['counseling_id']; ?>" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="emailDetailModalLabel<?= $session['counseling_id']; ?>">Email Address</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <p><strong>Email:</strong> <?= htmlspecialchars($session['email'] ?? 'N/A'); ?></p>
                    </div>
...
<td><?= htmlspecialchars($session['counselors_name']) ?: 'No Counselor yet' ?></td>
...
<div class="modal fade" id="violationModal<?= $session['counseling_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="violationModalLabel<?= $session['counseling_id']; ?>" aria-hidden="true">
    ...
    if (!empty($studentViolations[0])) {
        for ($i = 0; $i < count($studentViolations); $i++) {
            echo '<p><strong>Type:</strong> ' . htmlspecialchars(trim($studentViolations[$i])) . '</p>';
...
<td class="status-<?php echo strtolower(htmlspecialchars($session['status'])); ?>">
    ...
    if ($session['status'] === 'Scheduled') {
        $formattedScheduleTime = isset($session['schedule_time']) && !empty($session['schedule_time']) 
            ? date('Y-m-d h:i A', strtotime($session['schedule_time'])) 
            : '';
        ?>
        <div class="d-flex align-items-center">
            <?php echo htmlspecialchars($session['status']) . ($formattedScheduleTime ? ' (' . $formattedScheduleTime . ')' : ''); ?>
            <svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" fill="#3bb0de" class="bi bi-bell-fill ml-3 send-email-notification" viewBox="0 0 16 16" 
                data-email="<?= $session['email']; ?>"
                data-name="<?= $session['student_full_name']; ?>"
                data-schedule="<?= $formattedScheduleTime; ?>"
                style="cursor: pointer; filter: drop-shadow(2px 2px 2px rgba(0,0,0,0.3)); transition: all 0.3s ease;">
            ...
            <div class="modal-body">
                <form id="emailForm">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control rounded-0" id="studentName" name="student_name" readonly>
                    </div>
                    
                    <div class="form-group">
                        <label for="scheduleTime">Schedule</label>
                        <input type="text" class="form-control rounded-0" id="scheduleTime" name="schedule_time" readonly>
                    </div>
                    
                    <div class="form-group">
                        <label for="emailSubject">Email Subject</label>
                        <input type="text" class="form-control rounded-0" id="emailSubject" name="email_subject" value="Counseling Session Reminder">
                    </div>
                    
                    <div class="form-group">
                        <label for="emailMessage">Email Message</label>
                        <textarea class="form-control" id="emailMessage" name="email_message" rows="5"></textarea>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
...
<td class="action-buttons" style="justify-content: center;">
                <a href="edit_counseling.php?id=<?= $session['counseling_id']; ?>" data-toggle="tooltip" data-placement="top" title="Edit this student" class="edit-btn btn btn-md btn-outline-success">
...
function showAlert(message, alertType) {
    ...
    alertDiv.classList.add(alertType); // Add the appropriate alert type
    alertDiv.innerText = message; // Set the message
    alertDiv.style.display = 'block'; // Show the aalert
...
<script>
$(document).ready(function() {
    $('.send-sms-notification').click(function(e) {
        e.preventDefault();
        ...
        $('#scheduleTime').val(schedule);
        
        // Show modal
        $('#smsNotificationModal').modal('show');
    });
    ...
    const schedule = $('#scheduleTime').val();
    
    // Basic validation
    if (!phone) {
        showAlert('Phone number is required', 'custom-alert-warning');
        return;
    }
    ...
    $.ajax({
        ...
        data: {
            phone: phone,
            name: name || 'Student', // Default if empty
            schedule: schedule || 'Walk-in' // Default if empty
        },
        dataType: 'json',
        success: function(response) {
            // Reset button
            $btn.prop('disabled', false).html('Send SMS');
            
            if (response && response.success) {
                $('#smsNotificationModal').modal('hide');
                showAlert('SMS notification sent successfully!', 'custom-alert-success');
            } else {
                const errorMsg = response && response.message ? response.message : 'Failed to send SMS notification.';
                showAlert(errorMsg, 'custom-alert-danger');
            }
        },
        error: function(xhr, status, error) {
            ...
            let errorMsg = 'Error sending SMS. Please try again.';
            try {
                if (xhr.responseText) {
...
$(document).ready(function() {
    ...
    $('.send-email-notification').click(function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    showEmailSuccessNotification(name, email, schedule);
                } else {
                    // Show error message
                    showEmailErrorNotification(response.message);
                }
            },
            error: function() {
                // Re-enable button
                $btn.prop('disabled', false).text('Send Email');
                showEmailErrorNotification('An error occurred while sending the email. Please try again.');
            }
        });
    });
});

// Function to show custom email success notification
function showEmailSuccessNotification(name, email, schedule) {
    // Create the notification container
    const notification = $('<div>', {
        class: 'email-success-notification',
        css: {
            position: 'fixed',
            top: '50%',
            left: '50%',
    ...
    const header = $('<div>', {
        ...
        $('<button>', {
            ...
            click: function() {
                closeEmailNotification(notification, overlay);
            }
        })
    );
    ...
    const content = $('<div>', {
        ...
        $('<div>', {
            ...
            $('<p>', {
                html: '<strong>Recipient:</strong> ' + name,
                css: { margin: '5px 0' }
            }),
            $('<p>', {
            ...
            $('<p>', {
                html: '<strong>Sent at:</strong> ' + new Date().toLocaleTimeString() + ' on ' + new Date().toLocaleDateString(),
                css: { margin: '5px 0' }
            })
        )
    );
    ...
    notification.append(header, content, footer);
...
// Function to show email error notification
function showEmailErrorNotification(message) {
    const errorNotification = $('<div>', {
        class: 'email-error-notification',
        css: {
            position: 'fixed',
            top: '20px',
            right: '20px',
    ...
    // Show with animation
    setTimeout(function() {
        errorNotification.css('transform', 'translateX(0)');
    }, 10);
    
    // Auto-hide after 5 seconds
    setTimeout(function() {
        errorNotification.css('transform', 'translateX(400px)');
        setTimeout(function() {
...
// Function to close email notification
function closeEmailNotification(notification, overlay) {
    notification.css({
        opacity: 0,
